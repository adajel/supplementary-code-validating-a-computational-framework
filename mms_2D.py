...
class MMS:
    ...
    def get_src_term_k(self, alpha_r, k_r, phi_r, D_r, z_r, memflux):
        ...
        F = self.F; R = self.R; temperature = self.temperature
        # calculate gradients
        grad_k_r = np.array([sm.diff(k_r, x),  sm.diff(k_r, y)])
        grad_phi_r = np.array([sm.diff(phi_r, x), sm.diff(phi_r, y)])
        ...
        f = sm.diff(alpha_r*k_r, t) + sm.diff(J_k_r[0], x)  \
          + sm.diff(J_k_r[1], y) + memflux
        # return source term and adjusted compartmental ion flux
        return f, J_k_r
    ...
    def get_MMS_terms(self, params, degree):
        ...
        #calculate gradients
        grad_phi_N = np.array([sm.diff(phi_N_e, x), sm.diff(phi_N_e, y)])
        grad_phi_E = np.array([sm.diff(phi_E_e, x), sm.diff(phi_E_e, y)])
        ...
        D_Cl_ = params['D'][2]
        xie_N_ = params['xie'][0]
        gamma_NE_ = params['gamma_M'][0]
        nw_NE_ = params['nw_M'][0]
        C_NE_ = params['C_M'][0]

        g_Na_leak_N_ = params['g_Na_leak_N']
        g_K_leak_N_ = params['g_K_leak_N']
        g_Cl_leak_N_ = params['g_Cl_leak_N']

        time = self.time

        # convert exact solutions to Expressions
        alphaNe, NaNe, NaEe, KNe, KEe, ClNe, ClEe, phiNe, phiEe  = \
                [Expression(sm.printing.ccode(foo), t=time, degree=4)
                    for foo in (alpha_N_e, Na_N_e, Na_E_e, K_N_e, K_E_e, \
                            Cl_N_e, Cl_E_e, phi_N_e, phi_E_e)]

        falphaN, fNaN, fNaE, fKN, fKE, fClN, fClE, fphiN, fphiE = \
                [Expression(sm.printing.ccode(foo), \
                    z_Na=z_Na_, z_K=z_K_, z_Cl=z_Cl_, z_0=z_0_, \
                    D_Na=D_Na_, D_K=D_K_, D_Cl=D_Cl_, \
                    F=F_, R=R_, temperature=temperature_,\
                    C_NE=C_NE_, nw_NE=nw_NE_, xie_N=xie_N_, gamma_NE=gamma_NE_, \
                    g_Na_leak_N=g_Na_leak_N_, \
                    g_K_leak_N=g_K_leak_N_, \
                    g_Cl_leak_N=g_Cl_leak_N_,
                    t=time, degree=4)
        ...
        JNaN, JNaE, JKN, JKE, JClN, JClE  = \
                [Expression((sm.printing.ccode(foo[0]), sm.printing.ccode(foo[1])),\
                    z_Na=z_Na_, z_K=z_K_, z_Cl=z_Cl_, z_0=z_0_, F=F_, R=R_,
                    temperature=temperature_, D_Na=D_Na_, D_K=D_K_, D_Cl=D_Cl_,
                    xie_N=xie_N_, t=time, degree=4)
        ...
        src_terms = [falphaN, fNaN, fNaE, fKN, fKE, fClN, fClE, fphiN, fphiE]
    ...
    def get_exact_solution(self):
        ...
        # volume fraction
        alpha_N_e = 0.3 - 0.1*sm.sin(2*pi*x)*sm.sin(2*pi*y)*sm.exp(-t) # intracellular

        # sodium (Na) concentration
        Na_N_e = 0.1 + 0.3*sm.sin(pi*y*x)*sm.exp(-t) # intracellular
        Na_E_e = 0.3 + 0.6*sm.sin(pi*y*x)*sm.exp(-t) # extracellular

        # potassium (K) concentration
        K_N_e = 0.3 + 0.3*sm.sin(pi*y*x)*sm.exp(-t) # intracellular
        K_E_e = 0.2 + 0.2*sm.sin(pi*y*x)*sm.exp(-t) # extracellular

        # chloride (Cl) concentration
        Cl_N_e = 0.6 + 0.6*sm.sin(pi*y*x)*sm.exp(-t) # intracellular
        Cl_E_e = 0.1 + 0.8*sm.sin(pi*y*x)*sm.exp(-t) # extracellular

        # potential
        phi_N_e = sm.sin(2*pi*x)*sm.sin(2*pi*y)*sm.exp(-t)       # intracellular
        phi_E_e = sm.sin(2*pi*x)*sm.sin(2*pi*y)*(1 + sm.exp(-t)) # extracellular
    ...
    def get_initial_conditions(self):
        ...
        x = self.x; y = self.y; t = self.t
        # volume fraction
        alpha_N_init = 0.3 - 0.1*sm.sin(2*pi*x)*sm.sin(2*pi*y) # intracellular
        # sodium (Na) concentration
        Na_N_init = 0.1 + 0.3*sm.sin(pi*y*x) # intracellular
        Na_E_init = 0.3 + 0.6*sm.sin(pi*y*x) # extracellular
        # potassium (K) concentration
        K_N_init  = 0.3 + 0.3*sm.sin(pi*y*x) # intracellular
        K_E_init  = 0.2 + 0.2*sm.sin(pi*y*x) # extracellular
        # chloride (Cl) concentration
        Cl_N_init = 0.6 + 0.6*sm.sin(pi*y*x) # intracellular
        Cl_E_init = 0.1 + 0.8*sm.sin(pi*y*x) # extracellular
        # potential
        phi_N_init = sm.sin(2*pi*x)*sm.sin(2*pi*y)  # intracellular
        phi_E_init = sm.sin(2*pi*x)*sm.sin(2*pi*y)*2 # extracellular
...
class ProblemMMS():
    ...
    def __init__(self, mesh, boundary_point, t):
        self.mesh = mesh                     # mesh
        self.boundary_point = boundary_point # point to pin phi_E to zero
        self.t = t              # time constant (for updating source and boundary terms)
    ...
    def set_immobile_ions(self):
        ...
        # unwrap initial conditions
        alphaNinit = self.initial_conditions['alphaNinit']
        NaNinit = self.initial_conditions['NaNinit']
        NaEinit = self.initial_conditions['NaEinit']
        KNinit = self.initial_conditions['KNinit']
        KEinit = self.initial_conditions['KEinit']
        ClNinit = self.initial_conditions['ClNinit']
        ClEinit = self.initial_conditions['ClEinit']
        phiNinit = self.initial_conditions['phiNinit']
        phiEinit = self.initial_conditions['phiEinit']
